...
//' Obtain a standardization factor for the eigenvalues according to Eq. 18
 ...
 double EigenNorm(arma::colvec nu, int k){
     arma::colvec nu_red;
     if (k != 0){
 ...
 arma::mat StandEigVals (arma::mat X, int T, int N, double delta){
     ...
     // Eq. 20
     arma::colvec phi_1 = exp(pow(N, -delta) * nu_1 / nu_3_bar);
     // Eq. 25
     arma::colvec phi_2 = exp(pow(N, -delta) * log(log(T)) * nu_2 / nu_3_bar);
     arma::colvec phi_3 = exp(pow(N, -delta) * nu_3 / nu_3_bar);
 ...
 float RandomTest (arma::mat Phi, int indx, int p, int R, float u){
     float phi = Phi.col(indx - 1)(p - 1);
     // Step A1.1
     arma::vec xi(R, arma::fill::randn);
     // Step A1.2
     arma::uvec zeta = arma::conv_to<arma::uvec>::from(xi * phi <= u);
     // Step A1.3
     float zeta_sum = arma::accu(zeta);
 ...
 int RandomTestWrapper(arma::mat Phi, int indx, int rmax, float sig, int R, float u){
     int r_hat = 0;
 ...
 NumericVector nFactors (arma::mat X, int rmax, double alpha){
     ...
     arma::mat Phi = StandEigVals(X, T, N, delta);
