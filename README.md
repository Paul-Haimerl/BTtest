...
nonstationary panels using the routine proposed in Barigozzi & Trapani
...
more conservative (`BT1 = FALSE`) eigenvalue scaling:

``` r
BTtest(X = X, r_max = 10, alpha = 0.05, BT1 = TRUE)
```

Differences between `BT1 = TRUE/ FALSE` quickly vanish when the panel
includes more than 200 time periods ([Barigozzi & Trapani
